 ...
 */
static int pcxhr_check_reg_bit(struct pcxhr_mgr *mgr, unsigned int bar, unsigned int reg,
			     ...
			     unsigned int itdsp, int atomic)
{
	...
	if (itdsp & PCXHR_MASK_IT_MANAGE_HF5) {
		...
		PCXHR_OUTPL(mgr, PCXHR_PLX, PCXHR_PLX_MBOX0,
			    PCXHR_INPL(mgr, PCXHR_PLX, PCXHR_PLX_MBOX0) &
			    ~PCXHR_MBOX0_HF5);
	}
	...
	reg = (unsigned char)(((itdsp & PCXHR_MASK_EXTRA_INFO) >> 1) |
			      PCXHR_CVR_HI08_HC);
	PCXHR_OUTPB(mgr, PCXHR_DSP, PCXHR_DSP_CVR, reg);
	...
	if (itdsp & PCXHR_MASK_IT_MANAGE_HF5) {
		/* wait for hf5 bit */
		err = pcxhr_check_reg_bit(mgr, PCXHR_PLX, PCXHR_PLX_MBOX0,
					  PCXHR_MBOX0_HF5,
					  PCXHR_MBOX0_HF5,
					  PCXHR_TIMEOUT_DSP,
					  &reg);
...
void pcxhr_reset_xilinx_com(struct pcxhr_mgr *mgr)
{
	if (pcxhr_is_device_compatible_IE (mgr->pci->subsystem_device, PCXHR_DEVICE_IE5_ID)){
		...
		PCXHR_OUTPL(mgr, PCXHR_PLX, PCXHR_PLX_CHIPSC,chipsc);
		mdelay(200);
		...
		/* reset second xilinx */
		PCXHR_OUTPL(mgr, PCXHR_PLX, PCXHR_PLX_CHIPSC,
...
{
	unsigned int reg = PCXHR_INPL(mgr, PCXHR_PLX, PCXHR_PLX_IRQCS);
	/* enable/disable interrupts */
	if (enable)
		reg |=  (PCXHR_IRQCS_ENABLE_PCIIRQ | PCXHR_IRQCS_ENABLE_PCIDB);
	else
		reg &= ~(PCXHR_IRQCS_ENABLE_PCIIRQ | PCXHR_IRQCS_ENABLE_PCIDB);
	PCXHR_OUTPL(mgr, PCXHR_PLX, PCXHR_PLX_IRQCS, reg);
}
...
void pcxhr_reset_dsp(struct pcxhr_mgr *mgr)
{
	...
	/* let's reset the DSP */
	PCXHR_OUTPB(mgr, PCXHR_DSP, PCXHR_DSP_RESET, 0);
	msleep( PCXHR_WAIT_DEFAULT ); /* wait 2 msec */
	PCXHR_OUTPB(mgr, PCXHR_DSP, PCXHR_DSP_RESET, 3);
	msleep( PCXHR_WAIT_DEFAULT ); /* wait 2 msec */

	/* reset mailbox */
	PCXHR_OUTPL(mgr, PCXHR_PLX, PCXHR_PLX_MBOX0, 0);
}
...
static int pcxhr_download_dsp(struct pcxhr_mgr *mgr, const struct firmware *dsp)
{
	...
	for (i = 0; i < dsp->size; i += 3) {
		...
		/* send host data */
		PCXHR_OUTPB(mgr, PCXHR_DSP, PCXHR_DSP_TXH, data[0]);
		PCXHR_OUTPB(mgr, PCXHR_DSP, PCXHR_DSP_TXM, data[1]);
		PCXHR_OUTPB(mgr, PCXHR_DSP, PCXHR_DSP_TXL, data[2]);
			     ...
			     const struct firmware *eeprom)
{
	...
	if (PCXHR_INPL(mgr, PCXHR_PLX, PCXHR_PLX_MBOX0) & PCXHR_MBOX0_BOOT_HERE) {
		/* no need to load the eeprom binary,
		 * but init the HI08 interface
		 */
		PCXHR_OUTPB(mgr, PCXHR_DSP, PCXHR_DSP_ICR, reg | PCXHR_ICR_HI08_INIT);
		msleep(PCXHR_WAIT_DEFAULT);
		PCXHR_OUTPB(mgr, PCXHR_DSP, PCXHR_DSP_ICR, reg);
		msleep(PCXHR_WAIT_DEFAULT);
	...
	if (err)
		return err;
	/* wait for chk bit */
	return pcxhr_check_reg_bit(mgr, PCXHR_DSP, PCXHR_DSP_ISR, PCXHR_ISR_HI08_CHK,
...
int pcxhr_load_boot_binary(struct pcxhr_mgr *mgr, const struct firmware *boot)
{
	...
	if (err)
		return err;
	/* wait for hf5 bit */
	return pcxhr_check_reg_bit(mgr, PCXHR_PLX, PCXHR_PLX_MBOX0, PCXHR_MBOX0_HF5,
...
int pcxhr_load_dsp_binary(struct pcxhr_mgr *mgr, const struct firmware *dsp)
{
	...
	if (err)
		return err;
	/* wait for chk bit */
	return pcxhr_check_reg_bit(mgr, PCXHR_DSP, PCXHR_DSP_ISR,
				   PCXHR_ISR_HI08_CHK,
				   PCXHR_ISR_HI08_CHK,
				   PCXHR_TIMEOUT_DSP, &dummy);
}
...
static int pcxhr_send_msg_nolock(struct pcxhr_mgr *mgr, struct pcxhr_rmh *rmh)
{
	...
	/* wait for chk bit */
	err = pcxhr_check_reg_bit(mgr, PCXHR_DSP, PCXHR_DSP_ISR, PCXHR_ISR_HI08_CHK,
				  PCXHR_ISR_HI08_CHK, PCXHR_TIMEOUT_DSP, &reg);
	if (err)
		return err;
	/* reset irq chk */
	...
	if (err)
		return err;
	/* wait for chk bit == 0*/
	err = pcxhr_check_reg_bit(mgr, PCXHR_DSP, PCXHR_DSP_ISR, PCXHR_ISR_HI08_CHK, 0,
				  PCXHR_TIMEOUT_DSP, &reg);
	if (err)
		return err;
	...
	err = pcxhr_check_reg_bit(mgr, PCXHR_DSP, PCXHR_DSP_ISR, PCXHR_ISR_HI08_TRDY,
				  PCXHR_ISR_HI08_TRDY, PCXHR_TIMEOUT_DSP, &reg);
	if (err)
		return err;
	PCXHR_OUTPB(mgr, PCXHR_DSP, PCXHR_DSP_TXH, (data>>16)&0xFF);
	PCXHR_OUTPB(mgr, PCXHR_DSP, PCXHR_DSP_TXM, (data>>8)&0xFF);
	PCXHR_OUTPB(mgr, PCXHR_DSP, PCXHR_DSP_TXL, (data&0xFF));
...
{
	int start_mask = PCXHR_INPL(mgr, PCXHR_PLX, PCXHR_PLX_MBOX2);
	/* least significant 12 bits are the pipe states
...
static void pcxhr_update_timer_pos(struct pcxhr_mgr *mgr,
				   struct pcxhr_stream *stream,
				   int samples_to_add, unsigned long flags)
{
	if (stream->substream &&
...
irqreturn_t pcxhr_interrupt(int irq, void *dev_id)
{
	...
	mgr->dbg_cpt.interrupts_all++;

	//*************************ATTENTION*************************//
//	mgr->dbg_cpt.interrupts_all++;
//	mgr->dbg_cpt.interrupts_none++;
...
//	return IRQ_HANDLED;
	...
	if (irq) {
		reg = PCXHR_INPL(mgr, PCXHR_PLX, PCXHR_PLX_IRQCS);
	...
	/* clear interrupt */
	reg = 0;
	tmp = PCXHR_INPL(mgr, PCXHR_PLX, PCXHR_PLX_L2PCIDB);
	do {
		PCXHR_OUTPL(mgr, PCXHR_PLX, PCXHR_PLX_L2PCIDB, tmp);
		reg |= tmp;
		tmp = PCXHR_INPL(mgr, PCXHR_PLX, PCXHR_PLX_L2PCIDB);
	} while ( tmp != 0 );

	if ((reg & (PCXHR_IRQ_TIMER | PCXHR_IRQ_MASK)) == 0) {
		mgr->dbg_cpt.interrupts_dummy++;
	}
	/* timer irq occurred */
	if (reg & PCXHR_IRQ_TIMER) {
		...
		/* is a 24 bit counter */
		int dsp_time_new =
			PCXHR_INPL(mgr, PCXHR_PLX, PCXHR_PLX_MBOX4) & PCXHR_DSP_TIME_MASK;
