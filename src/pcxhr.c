...
static const struct pci_device_id pcxhr_ids[] = {
	...
	{ 0x10b5, 0x9056, 0x1369, 0xba41, 0, 0, PCI_ID_VX222E_IE5, },
	{ 0x10b5, 0x9056, 0x1369, 0xbb41, 0, 0, PCI_ID_PCX924E_IE5, },
...
[PCI_ID_VX222E_IE7] =      { "VX222e",       1, 1, 6, 44 },
[PCI_ID_PCX924E_IE7] =     { "PCX924e",      1, 1, 7, 44 },
[PCI_ID_VX222E_MIC_IE7] =  { "VX222e-Mic",   1, 1, 7, 44 },
[PCI_ID_PCX22E_IE7] =      { "PCX22e",       1, 0, 6, 44 },
...
static int pcxhr_hardware_timer(struct pcxhr_mgr *mgr, int start, unsigned int rate)
{
	struct pcxhr_rmh rmh;
	int err;
	//unsigned int regDoorbell, regIrq;
	...
	pcxhr_hr_timer_trig(mgr, start, rate);
	...
	do {
		...
		if (mgr->sample_rate != subs->runtime->rate) {
			...
			if (mgr->sample_rate == 0)
				/* start the DSP-timer */
				err = pcxhr_hardware_timer(mgr, 1, subs->runtime->rate);
...
static int pcxhr_close(struct snd_pcm_substream *subs)
{
	...
	if (--mgr->ref_count_rate == 0) {
		...
		pcxhr_hardware_timer(mgr, 0, 0);	/* stop the DSP-timer */
	}
			    ...
			    struct snd_info_buffer *buffer)
{
	...
	if (mgr->dsp_loaded & (1 << PCXHR_FIRMWARE_DSP_MAIN_INDEX)) {
		...
		snd_iprintf(buffer, "dsp time errors : %d\n",
			    mgr->dsp_time_err);
		snd_iprintf(buffer, "hrtimer calls : %d\n",
			    mgr->hr_timer_calls);
		...
		snd_iprintf(buffer, "IRQ ctrl : 0x%08x\n",
                            inl(mgr->port[1] + 0x68));
		snd_iprintf(buffer, "doorbell : 0x%08x\n",
                            inl(mgr->port[1] + 0x64));

		snd_iprintf(buffer, "User IO CONTROL : 0x%08x\n",
                            inl(mgr->port[1] + 0x6c));


		snd_iprintf(buffer, "@port 0 : 0x%lx\n",
                            mgr->port[0]);

//		snd_iprintf(buffer, "port 0 : 0x%08x:0x%08x:0x%08x:0x%08x\n",
//                            inl(mgr->port[0] + 0x0),
//                            inl(mgr->port[0] + 0x4),
//                            inl(mgr->port[0] + 0x8),
//                            inl(mgr->port[0] + 0xc));

		snd_iprintf(buffer, "@port 1 : 0x%lx\n",
                            mgr->port[1]);

		snd_iprintf(buffer, "@port 2 : 0x%lx\n",
                            mgr->port[2]);



		for(i = 0; i < 0x100; i+=4 ){
			snd_iprintf(buffer, "[0x%x(%i)]0x%08x\n",i,i,
	                            inl(mgr->port[1] + i));
		}
		...
		snd_iprintf(buffer, "dbg interrupts : \n\tall %d\n\tnone : %d\n\tfalse_none %d\n\thandled %d\n\tdummy %d\n\tirq %d\n\tasync %d\n\tincr %d\n\tloop %d\n",
				mgr->dbg_cpt.interrupts_all, mgr->dbg_cpt.interrupts_none, mgr->dbg_cpt.interrupts_false_irq_none,mgr->dbg_cpt.interrupts_handled, mgr->dbg_cpt.interrupts_dummy, mgr->dbg_cpt.interrupts_irq, mgr->dbg_cpt.interrupts_async, mgr->dbg_cpt.interrupts_incr, mgr->dbg_cpt.interrupts_loop);
		       ...
		       const struct pci_device_id *pci_id)
{
	...
	regIrq = PCXHR_INPL(mgr, PCXHR_PLX, PCXHR_PLX_IRQCS);
	regDoorbell = PCXHR_INPL(mgr, PCXHR_PLX, PCXHR_PLX_L2PCIDB);
	PCXHR_OUTPL(mgr, PCXHR_PLX, PCXHR_PLX_L2PCIDB, regDoorbell);
	printk(KERN_ERR "%s PCXHR_PLX_L2PCIDB 0x%x, regIrq 0x%x\n", __func__, regDoorbell, regIrq);
	...
	pcxhr_hr_timer_init(mgr);

//	msi = pci_enable_msi(pci);
//	printk(KERN_ERR "%s MSI = %d\n", __func__, msi);
