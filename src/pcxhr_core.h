...
//below hook in order to be able to dump register in pcxhr.c in /proc/asound/ file
/* registers used on the PLX (port 1), use PCXHR_PLX as bar */
...
/* registers used on the DSP (port 2), use PCXHR_DSP as bar */
...
/* registers used on the DSP and Xilinx (port 2), use PCXHR_DSP as bar : HR stereo cards only */
/* attention : byte access only, with inb and outb ! */
//#define PCXHR_DSP_RESET		0x20
...
#define PCXHR_INPB(mgr,bar,x)		inb( GET_ADDR(mgr, bar, x))
#define PCXHR_INPL(mgr,bar,x)		inl( GET_ADDR(mgr, bar, x))
#define PCXHR_OUTPB(mgr,bar,x,data)	outb( (data), GET_ADDR(mgr, bar, x) )
#define PCXHR_OUTPL(mgr,bar,x,data)	outl( (data), GET_ADDR(mgr, bar, x) )
